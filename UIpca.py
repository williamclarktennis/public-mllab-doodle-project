...
class DrawingApp:
    def __init__(self, root, num_categories = None, model = None, pca = None): 
    ...
    def update_bitmap(self):
        ...
        if(self.model is not None):
            ...
            # flatten the data: 
            bitmap_flat = bitmap.reshape(1,-1)
            # note that sklearn's pca already subtracts the training mean, so we don't need to do this.

            # end of preprocessing for pca --------------------

            # pca ---------------------------------------------

            bitmap_pca = self.pca.transform(bitmap_flat)

            # end of pca --------------------------------------

            # prediction using neural network model -----------

            prediction = self.model(bitmap_pca)

            # end of prediction using neural network model ----

            # The output has shape (1, num_categories)
            self.update_text(np.array(prediction).flatten())

        # Repeat every __ ms
        self.root.after(500, self.update_bitmap)
